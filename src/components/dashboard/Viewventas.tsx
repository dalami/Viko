...
function totalFijos(fijos: CostosFijos): number {
  return (
    (fijos.alquiler || 0) +
    (fijos.electricidad || 0) +
    (fijos.internet || 0) +
    (fijos.monotributo || 0) +
    (fijos.herramientas || 0) +
    (fijos.otros || 0)
  );
}
...
function calcularCostoFijoAsignado(
  fijos: CostosFijos,
  prod: CostoProducto,
): number {
...
function calcularCostoTotal(fijos: CostosFijos, prod: CostoProducto): number {
  return (
    totalVariables(prod.variables) + calcularCostoFijoAsignado(fijos, prod)
  );
}
...
function calcularGananciaJornada(
  jornada: Jornada,
  costos: CostosConfig,
): number {
  return jornada.items.reduce((total, item) => {
    const costoProd = costos.productos.find(
...
const VARIABLES_VACIO: CostosVariables = {
  materia_prima: 0,
  insumos: 0,
  packaging: 0,
  mano_de_obra: 0,
...
export default function ViewVentas({
  ...
  function compartirResumen(jornada: Jornada): void {
    ...
    const gananciaLinea = costos
      ...
      : "";
    const texto = [
      `🧾 Resumen — ${jornada.canal}`,
      `📅 ${fechaCorta(jornada.fecha)}`,
      "",
      ...lineas,
      "",
      `💰 Total: ${formatPeso(jornada.total)}${gananciaLinea}`,
      "",
      "Registrado con Viko 📲",
    ].join("\n");
    window.open(
  ...
  return (
    <div style={s.wrap}>
      {mostrarTabs && (
        <div style={s.tabs}>
          <button
            style={{ ...s.tab, ...(tab === "ventas" ? s.tabActive : {}) }}
            onClick={() => {
              setTab("ventas");
              setPantalla("home");
            }}
      ...
      {tab === "costos" ? (
        ...
        <>
          ...
          {pantalla === "registrar" && jornadaActiva && (
            <PantallaRegistrar
          ...
          {pantalla === "resumen" && (
            <PantallaResumen
              jornada={historial[0] ?? null}
              costos={costos}
              onCompartir={() => historial[0] && compartirResumen(historial[0])}
...
function TabCostos({
  ...
  const [fijos, setFijos] = useState<CostosFijos>(
    costos?.fijos ?? { ...FIJOS_VACIO },
  );
  const [reinversion, setReinversion] = useState(
    costos?.porcentaje_reinversion ?? 30,
  );
  const [prodCostos, setProdCostos] = useState<
    Record<string, { variables: CostosVariables; unidades: number }>
  >(() => {
    const map: Record<
      string,
      { variables: CostosVariables; unidades: number }
    > = {};
    costos?.productos?.forEach((c) => {
      map[c.productoId] = {
        variables: c.variables ?? { ...VARIABLES_VACIO },
        unidades: c.unidades_mes_estimadas ?? 1,
      };
    });
    return map;
  });
  ...
  const [prodExpandido, setProdExpandido] = useState<string | null>(
    productos[0]?.id ?? null,
  );
  ...
  function updateVariable(
    ...
    setProdCostos((prev) => ({
      ...
      [prodId]: {
        ...prev[prodId],
        variables: {
          ...(prev[prodId]?.variables ?? { ...VARIABLES_VACIO }),
          [campo]: valor,
        },
      },
    }));
  ...
  function updateUnidades(prodId: string, valor: number) {
    setProdCostos((prev) => ({
      ...prev,
      [prodId]: { ...prev[prodId], unidades: valor },
    }));
  }
  ...
  const unidadesMes = historial
    .filter((j) => j.fecha.startsWith(mesActual))
    .reduce((s, j) => s + j.items.reduce((si, i) => si + i.cantidad, 0), 0);
  const ventasMes = historial
    .filter((j) => j.fecha.startsWith(mesActual))
    .reduce((s, j) => s + j.total, 0);

  // Por producto: unidades necesarias para cubrir su parte de los fijos
  const equilibrioPorProducto = productos
    .map((p) => {
      const pc = prodCostos[p.id];
      if (!pc) return null;
      const safeVars = pc.variables ?? { ...VARIABLES_VACIO };
      const margenContrib = p.precio - totalVariables(safeVars);
      if (margenContrib <= 0) return null;
      const unidades = pc.unidades ?? 1;
      // Fijo asignado a este producto
      const totalUnidadesEstimadas = productos.reduce((s, pr) => {
        const prc = prodCostos[pr.id];
        return s + (prc?.unidades ?? 1);
      }, 0);
      const pesoRelativo = unidades / (totalUnidadesEstimadas || 1);
      const fijosAsignados = totalFijosMes * pesoRelativo;
      const unidadesNecesarias =
        fijosAsignados > 0 ? Math.ceil(fijosAsignados / margenContrib) : 0;
      const ventasNecesarias = unidadesNecesarias * p.precio;
      // Vendidas este mes de este producto
      const vendidas = historial
        .filter((j) => j.fecha.startsWith(mesActual))
        .reduce((s, j) => {
          const item = j.items.find((i) => i.productoId === p.id);
  ...
  // Global: suma de ventas necesarias por producto
  const ventasGlobalesNecesarias = equilibrioPorProducto.reduce(
    (s, p) => s + p.ventasNecesarias,
    0,
  );
  const unidadesGlobalesNecesarias = equilibrioPorProducto.reduce(
    (s, p) => s + p.unidadesNecesarias,
    0,
  );
  const puntoEq =
    totalFijosMes > 0 && unidadesGlobalesNecesarias > 0
      ? unidadesGlobalesNecesarias
      : null;
  ...
  return (
    <div>
      ...
      <div style={s.descBox}>
        ...
        <p style={s.descText}>
          Acá calculamos el <strong>costo real</strong> de cada producto sumando
          lo que gastás para producirlo (costos variables) más la parte de tus
          gastos mensuales que le corresponde (costos fijos). Con eso sabés
          exactamente cuánto ganás — o si estás perdiendo plata sin darte
          cuenta.
        </p>
      </div>
        ...
        <div style={s.sectionHeader}>
          ...
          <div>
            ...
            <p style={s.sectionSub}>
              Lo que pagás todos los meses, vendas mucho o poco.
            </p>
          </div>
        </div>
        <div style={s.infoBox}>
          <p style={s.infoText}>
            <strong>¿Qué es un costo fijo?</strong> Es un gasto que no cambia
            según cuánto producís. El alquiler, la luz, el internet — los pagás
            en enero aunque hayas vendido poco, y en diciembre aunque hayas
            vendido muchísimo. Son la base que tu negocio tiene que cubrir sí o
            sí.
          </p>
        </div>
        <div style={s.fijosGrid}>
          <NumInput
            label="🏢 Alquiler / uso del espacio"
            hint="Si trabajás desde casa, calculá un % proporcional de tu alquiler."
            value={fijos.alquiler}
            onChange={(v) => updateFijo("alquiler", v)}
            placeholder="Ej: 90000"
          />
          <NumInput
            label="💡 Electricidad general"
            hint="La luz del mes, no la de la máquina específica (eso va en variables)."
            value={fijos.electricidad}
            onChange={(v) => updateFijo("electricidad", v)}
            placeholder="Ej: 18000"
          />
          <NumInput
            label="📱 Internet / teléfono"
            hint="Conexión a internet, línea de celular, plataformas de gestión."
            value={fijos.internet}
            onChange={(v) => updateFijo("internet", v)}
            placeholder="Ej: 10000"
          />
          <NumInput
            label="🧾 Monotributo / impuestos"
            hint="Lo que pagás de monotributo o cualquier impuesto fijo mensual."
            value={fijos.monotributo}
            onChange={(v) => updateFijo("monotributo", v)}
            placeholder="Ej: 18000"
          />
          <NumInput
            label="🔧 Herramientas / mantenimiento"
            hint="Reparaciones, repuestos, suscripciones a herramientas de trabajo."
            value={fijos.herramientas}
            onChange={(v) => updateFijo("herramientas", v)}
            placeholder="Ej: 12000"
          />
          <NumInput
            label="➕ Otros gastos fijos"
            hint="Cualquier otro gasto mensual que no cambia con la producción."
            value={fijos.otros}
            onChange={(v) => updateFijo("otros", v)}
            placeholder="Ej: 5000"
          />
        </div>
      ...
      <div style={s.sectionBlock}>
        <div style={s.sectionHeader}>
          ...
          <div>
            ...
            <p style={s.sectionSub}>
              Lo que gastás cada vez que producís una unidad.
            </p>
          </div>
        </div>
        <div style={s.infoBox}>
          <p style={s.infoText}>
            <strong>¿Qué es un costo variable?</strong> Es un gasto que existe
            solo cuando producís. Si no hacés nada, no gastás nada en esto. La
            materia prima, el packaging, tu tiempo de trabajo — todo lo que va
            directamente en cada unidad que fabricás. A más producción, más
            gasto variable.
          </p>
        </div>
        ...
        {productos.length === 0 && (
          <p style={s.fieldHint}>
            No tenés productos cargados. Andá a la sección Productos para
            agregarlos.
          </p>
        )}
        ...
        {productos.map((p) => {
          ...
          const unidades = pc.unidades || 1;
          const costoFijoAsig =
            totalFijosMes > 0 ? totalFijosMes / unidades : 0;
          const costoTotalUnit = totalVars + costoFijoAsig;
          const margen = p.precio - costoTotalUnit;
          const margenPct =
            p.precio > 0 ? Math.round((margen / p.precio) * 100) : 0;
          ...
          return (
            <div key={p.id} style={s.prodBlock}>
              ...
              <button
                style={s.prodBlockHeader}
                onClick={() => setProdExpandido(expandido ? null : p.id)}
              >
                <div style={s.prodBlockLeft}>
                  ...
                  <div>
                    ...
                    <p style={s.prodBlockPrecio}>
                      Precio de venta: {formatPeso(p.precio)}
                    </p>
                  </div>
                </div>
                <div style={s.prodBlockRight}>
                  {totalVars > 0 && (
                    <span
                      style={{
                        ...s.margenPill,
                        background: margen > 0 ? "#e8f5e2" : "#fde8e8",
                        color: margen > 0 ? "#3a6b1e" : "#b91c1c",
                      }}
                    >
              ...
              {expandido && (
                <div style={s.prodBlockBody}>
                  ...
                  <div style={s.unidadesRow}>
                    <div>
                      <p style={s.numInputLabel}>
                        🏭 ¿Cuántas unidades producís por mes?
                      </p>
                      <p style={s.numInputHint}>
                        <strong>Importante:</strong> Este dato es solo para
                        repartir los costos fijos entre cada unidad que
                        fabricás. No es lo mismo que cuántas vendés — ese número
                        lo registrás en la pestaña Ventas. Ejemplo: si producís
                        50 al mes y tus fijos son $100.000, le corresponden
                        $2.000 a cada unidad producida.
                      </p>
                    </div>
                    <input
                      style={{
                      ...
                      value={unidades || ""}
                      onChange={(e) =>
                        updateUnidades(p.id, parseInt(e.target.value) || 1)
                      }
                      placeholder="Ej: 50"
                    />
                  </div>
                  ...
                  <div style={s.costeoResumen}>
                    ...
                    <div style={s.costeoRow}>
                      <span style={s.costeoLbl}>
                        Costo fijo asignado
                        <span style={s.costeoSub}>
                          {" "}
                          ({formatPeso(totalFijosMes)} ÷ {unidades} unidades que
                          producís)
                        </span>
                      </span>
                      <span style={s.costeoVal}>
                        {formatPeso(costoFijoAsig)}
                      </span>
                    </div>
                    <div
                      style={{
                        ...s.costeoRow,
                        borderTop: "2px solid #e0e0d8",
                        paddingTop: 8,
                        marginTop: 4,
                      }}
                    >
                    ...
                    {margen <= 0 && (
                      ...
                      >
                        El precio de venta no alcanza a cubrir todos los costos.
                        Revisá si podés bajar costos o aumentar el precio.
                      </p>
                    )}
                    {margen > 0 && margenPct < 20 && (
                      ...
                      >
                        El margen es positivo pero ajustado. Considerá que parte
                        va a reinversión — asegurate de que la ganancia neta te
                        sirva.
                      </p>
                    )}
                  </div>
      ...
      <div style={s.sectionBlock}>
        <div style={s.sectionHeader}>
          ...
          <div>
            ...
            <p style={s.sectionSub}>
              ¿Cuánto del margen va al negocio y cuánto es tuyo?
            </p>
          </div>
        </div>
        <div style={s.infoBox}>
          <p style={s.infoText}>
            <strong>¿Por qué reinvertir?</strong> El margen no es todo ganancia.
            Parte de ese dinero hay que guardarlo para reponer materiales que se
            pierden, productos que salen defectuosos, mejorar herramientas o
            crecer. La reinversión es lo que le da sustentabilidad al negocio.
            La ganancia neta es lo que efectivamente te llevás a casa.
          </p>
        </div>
        <div style={s.reinversionRow}>
          <div style={{ flex: 1 }}>
            <label style={s.numInputLabel}>
              % que reinvertís en el negocio
            </label>
            <p style={s.numInputHint}>
              De cada $100 de margen, ¿cuántos vuelven al negocio? Lo habitual
              es entre 20% y 50%.
            </p>
            <div style={s.inputWithPrefix}>
              <span style={s.inputPrefix}>%</span>
              <input
                ...
                value={reinversion || ""}
                onChange={(e) =>
                  setReinversion(parseFloat(e.target.value) || 0)
                }
                placeholder="30"
              />
            </div>
          ...
          <div style={s.reinversionVisual}>
            ...
            <div
              style={{
                ...s.reinversionBar,
                background: "#e8c840",
                width: `${100 - reinversion}%`,
              }}
            />
            <div style={s.reinversionLeyenda}>
              <span style={{ color: "#5a6e45", fontSize: 12 }}>
                🔄 {reinversion}% negocio
              </span>
              <span style={{ color: "#7a6020", fontSize: 12 }}>
                💰 {100 - reinversion}% tuyo
              </span>
            </div>
          </div>
        </div>
      ...
      {/* ── PUNTO DE EQUILIBRIO ────────────────────────────────────────────── */}
      {puntoEq !== null && totalFijosMes > 0 && (
        <div
          style={{
            ...s.sectionBlock,
            border: `1px solid ${ventasMes >= ventasGlobalesNecesarias ? "#5a6e45" : "#e0a800"}`,
          }}
        >
          <div style={s.sectionHeader}>
            ...
            <div>
              ...
              <p style={s.sectionSub}>
                ¿Cuánto tenés que vender para no perder plata?
              </p>
            </div>
          </div>
          ...
          <div style={s.infoBox}>
            <p style={s.infoText}>
              <strong>¿Qué es el punto de equilibrio?</strong> Es el mínimo que
              tenés que vender en el mes para cubrir todos tus costos fijos y no
              perder dinero. Si vendés exactamente eso, no perdés pero tampoco
              ganás. Recién por encima de ese número empieza la ganancia real.
              Es la meta mínima que tu negocio tiene que alcanzar cada mes.
            </p>
          </div>

          {/* Global */}
          <p style={s.varsTitle}>📊 En total, tu negocio necesita por mes:</p>
          <div style={s.equilibrioGrid}>
            <div style={s.equilibrioCard}>
              <span style={s.equilibrioBig}>{puntoEq}</span>
              <span style={s.equilibrioLbl}>
                unidades mínimas a vender por mes
              </span>
            </div>
            <div style={s.equilibrioCard}>
              <span style={{ ...s.equilibrioBig, fontSize: 20 }}>
                {formatPeso(ventasGlobalesNecesarias)}
              </span>
              <span style={s.equilibrioLbl}>
                en ventas para cubrir costos fijos
              </span>
            </div>
          </div>
          ...
          {historial.length > 0 && (
            <>
              <p style={{ ...s.varsTitle, marginTop: 16 }}>
                📅 Este mes llevás:
              </p>
              <div style={s.equilibrioGrid}>
                <div
                  style={{
                    ...s.equilibrioCard,
                    border: `1px solid ${unidadesMes >= puntoEq ? "#5a6e45" : "#e0a800"}`,
                  }}
                >
                  <span
                    style={{
                      ...s.equilibrioBig,
                      color: unidadesMes >= puntoEq ? "#3a6b1e" : "#a06000",
                    }}
                  >
                    {unidadesMes}
                  </span>
                  <span style={s.equilibrioLbl}>
                    unidades vendidas este mes (registradas en Ventas)
                  </span>
                </div>
                <div
                  style={{
                ...
                >
                  <span
                    style={{
                      ...
                      fontSize: 20,
                      color:
                        ventasMes >= ventasGlobalesNecesarias
                          ? "#3a6b1e"
                          : "#a06000",
                    }}
                  >
                    {formatPeso(ventasMes)}
                  </span>
                  <span style={s.equilibrioLbl}>facturado este mes</span>
                </div>
              </div>
              <div
                style={{
                  ...
                  background:
                    ventasMes >= ventasGlobalesNecesarias
                      ? "#e8f5e2"
                      : "#fffbf0",
                  border: `1px solid ${ventasMes >= ventasGlobalesNecesarias ? "#9fcf7a" : "#f5d88e"}`,
                }}
              >
                <p
                  style={{
                ...
                >
                  {ventasMes >= ventasGlobalesNecesarias
                    ? `✅ ¡Superaste el punto de equilibrio este mes! Todo lo que vendas de acá en adelante es ganancia neta.`
                    : `⚠️ Te faltan ${formatPeso(ventasGlobalesNecesarias - ventasMes)} en ventas para cubrir tus costos fijos de este mes.`}
                </p>
              </div>
            </>
          )}
          ...
          {/* Por producto */}
          {equilibrioPorProducto.length > 0 && (
            <>
              <p style={{ ...s.varsTitle, marginTop: 20 }}>
                🏷️ Por producto — ¿cuánto necesitás vender de cada uno?
              </p>
              <div style={s.infoBox}>
                <p style={s.infoText}>
                  Estos números muestran cuántas unidades de cada producto tenés
                  que vender para que ese producto cubra la parte que le
                  corresponde de tus gastos fijos. Son una guía para saber dónde
                  enfocarte durante el mes.
                </p>
              </div>
              {equilibrioPorProducto.map((prod) => {
                const ok = prod.vendidas >= prod.unidadesNecesarias;
                const pct =
                  prod.unidadesNecesarias > 0
                    ? Math.min(
                        100,
                        Math.round(
                          (prod.vendidas / prod.unidadesNecesarias) * 100,
                        ),
                      )
                    : 0;
                return (
                  <div key={prod.nombre} style={s.prodEqRow}>
                    <div style={s.prodEqInfo}>
                      <p style={s.prodEqNombre}>{prod.nombre}</p>
                      <p style={s.prodEqSub}>
                        {formatPeso(prod.precio)} c/u · margen de contribución{" "}
                        {formatPeso(Math.round(prod.margenContrib))}
                      </p>
                    </div>
                    <div style={s.prodEqMeta}>
                      <span
                        style={{
                          ...s.prodEqNum,
                          color: ok ? "#3a6b1e" : "#1a1a1a",
                        }}
                      >
                        {historial.length > 0
                          ? `${prod.vendidas} vendidas / `
                          : ""}
                        {prod.unidadesNecesarias} necesarias
                      </span>
                      <span style={s.prodEqVentas}>
                        {formatPeso(prod.ventasNecesarias)}
                      </span>
                    </div>
                    {historial.length > 0 && (
                      <div style={s.prodEqBarWrap}>
                        <div
                          style={{
                            ...s.prodEqBar,
                            width: `${pct}%`,
                            background: ok ? "#5a6e45" : "#e0a800",
                          }}
                        />
                      </div>
                    )}
      ...
      <p style={{ ...s.fieldHint, textAlign: "center" as const, marginTop: 8 }}>
        Tus costos quedan guardados en Viko. La próxima vez que abras esta
        pantalla ya van a estar acá.
      </p>
    </div>
  );
}
...
function PantallaHome({
  ...
  const totalMes = historial
    .filter((j) => j.fecha.startsWith(mesActual))
    .reduce((s, j) => s + j.total, 0);
  const gananciaEstimadaMes = costos
    ? historial
        .filter((j) => j.fecha.startsWith(mesActual))
        .reduce((s, j) => s + calcularGananciaJornada(j, costos), 0)
    : null;
  ...
  return (
    <div>
      <div style={s.pageHeader}>
        ...
        <div style={s.descBox}>
          ...
          <p style={s.descText}>
            ...
            puerta a puerta — registrá tus productos con un toque. Con el tiempo
        ...
        <div style={s.statsRow}>
          ...
          {gananciaEstimadaMes !== null && (
            <div style={{ ...s.statCard, border: "1px solid #5a6e45" }}>
              <span style={{ ...s.statNum, color: "#3a6b1e" }}>
                {formatPeso(gananciaEstimadaMes)}
              </span>
      ...
      <div style={s.card}>
        ...
        <p style={s.helpText}>
          Escribí un nombre para identificar esta jornada. Puede ser el lugar
          (Feria de Palermo), el canal (Instagram, WhatsApp) o la fecha (Sábado
          31).
        </p>
            ...
            return (
              <div key={j.id} style={s.jornadaRow}>
                <div>
                  ...
                  {ganancia !== null && (
                    <p style={{ ...s.jornadaFecha, color: "#3a6b1e" }}>
                      Ganancia est.: {formatPeso(ganancia)}
                    </p>
                  )}
                </div>
      ...
      {historial.length === 0 && (
        <div style={s.emptyState}>
          ...
          <p style={s.emptySub}>
            Iniciá tu primera jornada arriba. No necesitás saber de tecnología —
            es tan simple como escribir dónde estás vendiendo y tocar cada
            producto que vendés.
          </p>
        </div>
      )}
  ...
  return (
    <div>
      <div style={s.jornadaHeader}>
        <div>
          ...
          <p style={s.jornadaHeaderSub}>
            Tocá un producto cada vez que vendés uno
          </p>
        </div>
      ...
      <div style={s.descBox}>
        <span style={s.descIcon}>👆</span>
        <p style={s.descText}>
          Cada toque suma una unidad vendida. Para descontar una venta
          equivocada, tocá el <strong>−</strong> en el producto.
        </p>
      </div>
      {productos.length > 5 && (
        <input
      ...
      <div style={s.productosGrid}>
        {prodsFiltrados.map((p) => {
          const cant =
            jornada.items.find((i) => i.productoId === p.id)?.cantidad ?? 0;
          const precio =
    ...
    <div>
      <div style={s.resumenHero}>
        ...
        {ganancia !== null && (
          ...
          >
            Ganancia estimada: {formatPeso(ganancia)}
          </p>
        )}
      </div>
      <div style={s.card}>
        ...
        {!costos && (
          <p style={{ ...s.fieldHint, marginTop: 12 }}>
            💡 Cargá tus costos en la pestaña <strong>Costos</strong> para ver
            cuánto ganaste en cada jornada.
          </p>
        )}
      </div>
    ...
    <div>
      ...
      {isPro && canales.length > 1 && (
        <div style={s.card}>
          ...
          <p style={s.helpText}>
            Cuánto vendiste en total en cada lugar o canal, del mejor al menor.
          </p>
          {mejorCanal.map((c) => (
      ...
      <div style={s.card}>
        ...
        <p style={s.helpText}>
          Tocá cualquier jornada para ver el detalle de lo que vendiste ese día.
        </p>

        {historial.map((j) => {
          const ganancia = costos ? calcularGananciaJornada(j, costos) : null;
          const abierta = expandida === j.id;
          const totalUnidades = j.items.reduce((s, i) => s + i.cantidad, 0);

          return (
            <div key={j.id} style={{ borderBottom: "1px solid #f0f0ec" }}>
              {/* Fila principal — siempre visible, tocable */}
              <button
                style={s.jornadaBtn}
                onClick={() => setExpandida(abierta ? null : j.id)}
              >
                <div style={{ flex: 1, textAlign: "left" as const }}>
                  <p style={s.jornadaNombre}>{j.canal}</p>
                  <p style={s.jornadaFecha}>
                    {fechaCorta(j.fecha)} · {totalUnidades} unidad
                    {totalUnidades !== 1 ? "es" : ""} · {j.items.length}{" "}
                    producto{j.items.length !== 1 ? "s" : ""}
                  </p>
                  {ganancia !== null && (
                    <p style={{ ...s.jornadaFecha, color: "#3a6b1e" }}>
                      Ganancia est.: {formatPeso(ganancia)}
              ...
              {/* Detalle expandible */}
              {abierta && (
                <div style={s.jornadaDetalle}>
                  <p style={s.jornadaDetalleTitulo}>Detalle de la jornada</p>

                  {j.items.map((item) => {
                    const costoItem = costos?.productos.find(
                      (c) => c.productoId === item.productoId,
                    );
                    const margenItem = costoItem
                      ? (item.precio - totalVariables(costoItem.variables)) *
                        item.cantidad
                      : null;
                    return (
                      <div key={item.productoId} style={s.detalleRow}>
                        <div style={{ flex: 1 }}>
                          <span style={s.detalleNombre}>{item.nombre}</span>
                          <span style={s.detallePrecio}>
                            {formatPeso(item.precio)} c/u
                          </span>
                        </div>
                        <div style={{ textAlign: "right" as const }}>
                          <span style={s.detalleCant}>×{item.cantidad}</span>
                          <span style={s.detalleTotal}>
                            {formatPeso(item.precio * item.cantidad)}
                          </span>
                          {margenItem !== null && (
                            <span
                              style={{
                                ...s.detallePrecio,
                                color: margenItem >= 0 ? "#3a6b1e" : "#b91c1c",
                                display: "block",
                              }}
                            >
                              {margenItem >= 0 ? "+" : ""}
                              {formatPeso(margenItem)} margen
                            </span>
                          )}
                        </div>
                      </div>
                  ...
                  {/* Total y acciones */}
                  <div style={s.detalleTotalRow}>
                    <span style={s.detalleTotalLbl}>Total de la jornada</span>
                    <span style={s.detalleTotalNum}>{formatPeso(j.total)}</span>
                  </div>
                  {ganancia !== null && (
                    <div
                      style={{
                    ...
                    >
                      <span style={{ ...s.detalleTotalLbl, color: "#3a6b1e" }}>
                        Ganancia estimada
                      </span>
                      <span style={{ ...s.detalleTotalNum, color: "#3a6b1e" }}>
                        {formatPeso(ganancia)}
                      </span>
                    </div>
                  )}
                  <button
                    style={{ ...s.btnWA, marginTop: 12 }}
                    onClick={() => onCompartir(j)}
                  >
                    💬 Compartir este resumen por WhatsApp
                  </button>
                </div>
              )}
            </div>
        ...
        {!isPro && historial.length >= 5 && (
          ...
          >
            Mostrando las últimas 5. Actualizá a Pro para ver el historial
            completo.
          </p>
        )}
      </div>
...
const s: Record<string, React.CSSProperties> = {
  ...
  prodEqSub: { fontSize: 11, color: "#999", margin: "2px 0 0" },
  prodEqMeta: {
    display: "flex",
    justifyContent: "space-between",
    alignItems: "baseline",
  ...
  prodEqNum: { fontSize: 14, fontWeight: 700 },
  prodEqVentas: { fontSize: 12, color: "#888" },
  prodEqBarWrap: {
    height: 6,
    background: "#f0f0ec",
    borderRadius: 3,
  ...
  prodEqBar: { height: "100%", borderRadius: 3, transition: "width .4s ease" },
  jornadaBtn: {
    width: "100%",
    display: "flex",
    alignItems: "center",
  ...
  detalleTotalLbl: { fontSize: 13, fontWeight: 600, color: "#555" },
  detalleTotalNum: { fontSize: 16, fontWeight: 800, color: "#5a6e45" },
};
