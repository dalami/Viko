...
export function ViewLanding({ emp, slug, isPro }: { emp: Emprendimiento; slug: string; isPro: boolean }) {
    ...
    <div className={styles.view}>
      ...
      <section className={styles.section}>
        ...
        <div className={styles.domainOption}>
          <div>
            ...
            <p className={styles.toggleSub}>Conectá tu propio dominio (ej: tuemprendimiento.com)</p>
          </div>
      ...
      <section className={styles.section}>
        ...
        <p className={styles.sectionSub}>
          Usalo en TikTok, Instagram, packaging o tarjetas. Cada escaneo llega directo a tu ficha.
        </p>
        <div style={{ display: "flex", gap: 32, alignItems: "flex-start", flexWrap: "wrap" }}>
          <div style={{
            background: "#FAFAF7", border: "1px solid rgba(26,24,20,0.12)",
            borderRadius: 16, padding: 20, display: "inline-block",
          }}>
            <canvas ref={canvasRef} style={{ display: "block", borderRadius: 8 }} />
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 12, paddingTop: 8 }}>
            <p style={{ fontSize: 13, color: "#7A756A", lineHeight: 1.6, maxWidth: 240 }}>
              Descargá el QR y usalo donde quieras. Cada escaneo queda registrado en tus métricas.
            </p>
            <button onClick={downloadQR} style={{
              background: "#1A1814", border: "none", borderRadius: 100,
              padding: "11px 24px", fontFamily: "Syne, sans-serif",
              fontWeight: 700, fontSize: 13, color: "#FAFAF7", cursor: "pointer",
              display: "inline-flex", alignItems: "center", gap: 8,
            }}>
              ⬇️ Descargar QR
            </button>
            <p style={{ fontSize: 11, color: "#9A958A" }}>Formato PNG · 200×200px</p>
          </div>
        </div>
      </section>
      ...
      <section className={styles.section}>
        ...
        <p className={styles.sectionSub}>Así ven tu página los clientes cuando entran al link.</p>
        <div className={styles.previewFrame}>
          <div className={styles.previewNav}>
            <span className={styles.previewLogo}>
              Viko<span style={{ color: "var(--olive)" }}>.</span>
            </span>
          </div>
          <div className={styles.previewHero}>
            <p className={styles.previewRubro}>{emp.rubro || "Categoría"}</p>
            <h2 className={styles.previewName}>{emp.nombre || "Tu emprendimiento"}</h2>
            <p className={styles.previewTagline}>{emp.tagline || "Tu tagline aparece acá"}</p>
            {emp.descripcion && (
              <p className={styles.previewTagline} style={{ fontSize: 13, opacity: 0.8, marginTop: -8 }}>
                {emp.descripcion}
              </p>
            )}
            <div className={styles.previewBtns}>
              <span className={styles.previewBtnWa}>WhatsApp</span>
              <span className={styles.previewBtnIg}>Instagram</span>
            </div>
          </div>
        </div>
      </section>
