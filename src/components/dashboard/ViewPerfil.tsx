...
function ProLock({ onUpgrade }: { onUpgrade: () => void }) {
  return (
    <div style={{
      ...
      <p style={{ color: "#C9A84C", fontWeight: 700, fontSize: 14, marginBottom: 4 }}>
        Función exclusiva Viko Pro
      </p>
      <p style={{ color: "rgba(255,255,255,0.5)", fontSize: 12, marginBottom: 16, lineHeight: 1.5 }}>
        Activá tu plan para acceder a WhatsApp, Instagram y más.
      </p>
      <button onClick={onUpgrade} style={{
        background: "#C9A84C", border: "none", borderRadius: 100,
        padding: "10px 24px", fontFamily: "Syne, sans-serif",
...
function ImageSlot({ index, label, images, uploading, onUpload, onRemove }: {
    ...
    <div style={{ position: "relative" }}>
      ...
      <div className={`${styles.imgSlot} ${imgUrl ? styles.imgFilled : ""}`}
        ...
        {isUploading ? (
          ...
          <img src={imgUrl} alt={label} className={styles.imgPreview} />
        ) : (
  ...
  return (
    <div className={styles.view}>
      ...
      <section className={styles.section}>
        ...
        <div className={styles.formGrid}>
          <div className={styles.field}>
            ...
            <input className="input-field" value={emp.nombre || ""}
              onChange={(e) => update("nombre", e.target.value)} placeholder="Nombre de tu marca" />
          </div>
          <div className={styles.field}>
            <label className="field-label">Categoría</label>
            <select className="input-field" value={emp.rubro || ""}
          ...
          <div className={styles.field}>
            ...
            <input className="input-field" value={emp.tagline || ""}
              onChange={(e) => update("tagline", e.target.value)} placeholder="Una frase que defina tu marca" />
          </div>
          <div className={styles.field}>
            <label className="field-label">Ubicación</label>
            <input className="input-field" value={emp.ubicacion || ""}
              onChange={(e) => update("ubicacion", e.target.value)} placeholder="Ciudad, Provincia" />
          </div>
        </div>
        <div className={styles.field}>
          <label className="field-label">Descripción</label>
          <textarea className="input-field" value={emp.descripcion || ""}
      ...
      <section className={styles.section}>
        ...
        {uploadError && (
          <p style={{ color: "#C4664A", fontSize: 13, marginBottom: 12 }}>⚠️ {uploadError}</p>
        )}
        {isPro ? (
          <>
        ...
        {isPro ? (
          <div className={styles.formGrid}>
            <div className={styles.field}>
              ...
              <input className="input-field" value={emp.whatsapp || ""}
                onChange={(e) => update("whatsapp", e.target.value)} placeholder="5491100000000" />
            </div>
            <div className={styles.field}>
              <label className="field-label">Instagram (sin @)</label>
              <input className="input-field" value={emp.instagram || ""}
                onChange={(e) => update("instagram", e.target.value)} placeholder="tuemprendimiento" />
            </div>
            <div className={styles.field}>
              <label className="field-label">Email</label>
              <input className="input-field" type="email" value={emp.email || ""}
                onChange={(e) => update("email", e.target.value)} placeholder="hola@tuemprendimiento.com" />
            </div>
            <div className={styles.field}>
              <label className="field-label">Sitio web</label>
              <input className="input-field" value={emp.web || ""}
                onChange={(e) => update("web", e.target.value)} placeholder="https://tuemprendimiento.com" />
            </div>
          </div>
        ) : (
