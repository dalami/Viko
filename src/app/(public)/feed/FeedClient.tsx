...
const TIPO_CONFIG = {
  win: {
    ...
    color: "#6B7A5A",
    bg: "#EFF2EB",
    desc: "Compartí un logro",
  },
  pregunta: {
    label: "❓ Pregunta",
    color: "#4A90D9",
    bg: "#EBF3FB",
    desc: "Preguntale a la comunidad",
  },
  tip: {
    label: "💡 Tip",
    color: "#C9A84C",
    bg: "#FBF5E8",
    desc: "Compartí algo que aprendiste",
  },
};
...
function validarPost(contenido: string): string | null {
  ...
  if (contenido.length > 300)
    return "El post no puede superar los 300 caracteres";
...
function getEmp(
  raw: Emprendimiento | Emprendimiento[] | null,
): Emprendimiento | null {
  if (!raw) return null;
    ...
    >
      ...
      <div style={{ flex: 1 }}>
        ...
        >
          ...
          <span style={{ fontSize: 11, color: "#9A958A" }}>
            {timeAgo(respuesta.created_at)}
          </span>
          {isAdmin && (
            <button
        ...
        <p style={{ fontSize: 13, color: "#2D2B26", lineHeight: 1.6 }}>
          {respuesta.contenido}
        </p>
      </div>
    </div>
...
function PostCard({
  ...
  async function handleResponder() {
    ...
    const { data, error } = await supabase
      ...
      .select(
        `id, contenido, created_at, emprendimientos (id, nombre, slug, images)`,
      )
      .single();
    ...
    >
        ...
        <div style={{ marginBottom: 12 }}>
          <button
            ...
            style={{
              ...
              fontWeight: 600,
              padding: 0,
              marginBottom: showRespuestas ? 12 : 0,
            }}
          >
            {showRespuestas
              ? "▲ Ocultar"
        ...
        >
          ...
          >
            ...
            <button
              ...
              disabled={savingResp || !textoRespuesta.trim()}
              style={{
                background:
                  savingResp || !textoRespuesta.trim() ? "#EDE8DC" : "#6B7A5A",
                color:
                  savingResp || !textoRespuesta.trim() ? "#9A958A" : "#FAFAF7",
                border: "none",
                borderRadius: 100,
                padding: "6px 16px",
                fontFamily: "'Syne', sans-serif",
      ...
      >
        ...
        <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
          ...
          {isAdmin && (
            <button
              onClick={() => onEliminarPost(post.id)}
              style={{
                background: "#C4664A",
                border: "none",
                borderRadius: 100,
...
export default function FeedClient({
  ...
  const [filtro, setFiltro] = useState<"todos" | "win" | "pregunta" | "tip">(
    "todos",
  );
  ...
  async function handlePost() {
    ...
    const { data, error: dbError } = await supabase
      ...
      .select(
        `id, tipo, contenido, likes, created_at, emprendimientos (id, nombre, rubro, slug, images)`,
      )
      .single();
    ...
    if (dbError) {
      ...
      setPosts((prev) => [
        { ...(data as unknown as Post), respuestas: [] },
        ...prev,
      ]);
      setContenido("");
    }
  ...
  async function handleReportar(postId: number) {
    ...
    await supabase
      .from("post_reportes")
      .insert({ post_id: postId, emprendimiento_id: miEmprendimiento?.id });
    ...
    <div style={{ minHeight: "100vh", background: "#F5F0E8" }}>
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <Link
            href="/directorio"
            style={{ fontSize: 13, color: "#7A756A", textDecoration: "none" }}
          >
            Directorio
          </Link>
          {isAdmin && (
      ...
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "40px 5vw" }}>
        ...
        <div style={{ marginBottom: 32 }}>
          ...
          <p style={{ fontSize: 15, color: "#7A756A", lineHeight: 1.6 }}>
            Wins, tips y preguntas de la comunidad. Solo emprendedores
            registrados pueden publicar.
          </p>
        </div>
        ...
        {miEmprendimiento ? (
          ...
          >
            ...
            >
              Publicar como{" "}
              <span style={{ color: "#6B7A5A" }}>
                {miEmprendimiento.nombre}
              </span>
            </p>
            <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
              {(
                Object.entries(TIPO_CONFIG) as [
                  typeof tipo,
                  (typeof TIPO_CONFIG)[typeof tipo],
                ][]
              ).map(([key, cfg]) => (
                <button
                  key={key}
            ...
            <textarea
              ...
              placeholder={TIPO_CONFIG[tipo].desc + "..."}
              maxLength={300}
              style={{
                width: "100%",
                minHeight: 90,
            ...
            >
              <span
                style={{
                  fontSize: 11,
                  color: contenido.length > 280 ? "#C4664A" : "#9A958A",
                }}
              >
                {contenido.length}/300
              </span>
              {error && (
                <span style={{ fontSize: 12, color: "#C4664A" }}>
                  ⚠️ {error}
                </span>
              )}
              <button
                ...
                style={{
                  background:
                    saving || !contenido.trim() ? "#EDE8DC" : "#1A1814",
          ...
          >
            <p style={{ fontSize: 14, color: "#7A756A", marginBottom: 14 }}>
              ¿Tenés un emprendimiento? Registrate para participar en la
              comunidad.
            </p>
            <Link
              href="/register"
